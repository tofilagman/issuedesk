...
{#if stats}
  <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
    {@render tile('Total issues', stats.total, 'text-slate-800 dark:text-slate-100')}
    ...
    {@render tile('Created (7d)', stats.createdLast7, 'text-slate-800 dark:text-slate-100')}
  ...
  <div class="mt-5 grid gap-5 lg:grid-cols-2">
    <div class="card p-5">
      <div class="mb-4 flex items-center">
        <h3 class="text-sm font-semibold text-slate-600 dark:text-slate-300">Workload by assignee</h3>
        <span class="ml-auto text-xs text-slate-400 dark:text-slate-500">{stats.unassigned} unassigned</span>
      </div>
      {#if assigneeBars.length === 0}
        <p class="text-xs text-slate-400 dark:text-slate-500">No assigned issues yet.</p>
      {:else}
        <BarList items={assigneeBars} />
      {/if}
    </div>
    <div class="card flex flex-col justify-center p-5">
      <h3 class="mb-3 text-sm font-semibold text-slate-600 dark:text-slate-300">At a glance</h3>
      <p class="text-sm text-slate-600 dark:text-slate-300">
        <span class="font-semibold text-slate-800 dark:text-slate-100">{stats.open}</span> open ·
